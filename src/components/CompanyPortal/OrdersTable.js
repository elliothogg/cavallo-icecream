...
function OrdersTable(props) {
  ...
  function showOrders() {
    if (props.ordersToShow === "All") {
      ...
      return props.orders.slice(0).reverse().map((item, index) => {
        console.log(props.startOfWeekDate + "   " + props.endOfWeekDate + "   " + item.OrderTime.slice(0,10))
    ...
    else if (props.ordersToShow === "Custom Range") {
      if (props.startDate === undefined) return;
      else if (!props.startDate === undefined && props.endDate === undefined) {
        console.log("----" + props.startDate + props.todaysDate);
        return props.orders.slice(0).reverse().map((item, index) => {
          if (props.startDate === props.todaysDate)
            return (
              <div className="col-20" key={index}>
                <div className="row" style={{ color: '#333333' }}>
                  <div className="col-2"><button onClick={() => props.onChange(item.OrderID)} style={{ color:'blue', 'textDecoration': 'underline'}}>{item.OrderID}</button></div>
  ...
  return (
      <div id="orders-table-container" className="container-fluid">
        ...
        <div className="row"><h3 className="mb-4 col-12">{props.ordersToShow}</h3></div>
